...
- 桌面端 Go 客户端继续替换早期遗留交互问题；本地已补充自动化自测隔离开关，并新增无托盘面板版用于系统策略拦截托盘组件时继续使用控制面板、同步、文件发送和拉取能力；在关闭本机安全软件拦截后，托盘版已可用临时配置启动、访问本地控制面板状态接口，并完成临时服务端 pending / approve / trusted / 手动文本发送闭环
