...
- Shizuku 这条后续计划已经重新收口成“辅助复制模式”而不是“后台主模式”：接下来要继续把 `Shizuku` 重新命名为 `Shizuku 辅助模式 / 桥接模式`，补齐独立开关、自动上传、轻量提示、失败回退到悬浮发送助手等配置，并把运行页状态改成“已就绪 / 仅部分就绪 / 需手动确认 / 当前回退到普通模式”这几类；文案与文档也要同步改成“尽量无感复制后上传、但不承诺绕过系统后台剪贴板限制”的口径，避免用户再把它理解成稳定的后台读剪贴板主通道
...
- 本轮还顺手把运行时主快捷动作再收了一层，减少“历史辅助能力像正式模式一样参与主流程”的错觉：`handleRuntimeModeQuickAction()` 和剪贴板诊断按钮现在继续只围绕前台服务 / 悬浮窗这两条正式模式主路径工作；旧 `accessibility / shizuku` 仍保留兼容诊断与授权入口，但不再让运行页主快捷动作像以前那样继续为它们维持一整套“当前正式模式就绪后的主按钮行为”
- 本轮继续把运行页摘要主语再压实了一层，避免视觉上仍把旧辅助能力读成“正式当前模式”：`buildClipboardModeAdvice()` 里无障碍兼容态现已统一写成“当前辅助状态”，Shizuku 相关现已统一写成“当前诊断状态”，无障碍那条后台复制就绪度也从原先容易误读的“较稳”改成了“辅助链路已就绪”。这样后续用户即使命中历史兼容态，运行页上看到的也会更清楚地区分“正式模式”和“辅助/诊断状态”
...
- 本轮继续把这条收口从“文案上不承诺”推进到“正式主链路里也不再偷偷做”：`SyncService.publishLocalClipboardIfNeeded(...)` 与 `readCurrentClipboardText()` 中原先仍会在 `clipboard_mode=shizuku` 下调用 `ShizukuClipboardReader.readText(...)` 的分支现已移除，正式文本读取重新统一回系统 `ClipboardManager` 与无障碍快照兜底；`ShizukuClipboardReader.kt`、`ShizukuClipboardProbe.kt` 等历史实现先继续保留为已探索证据和后续有序清理素材，但不再挂在当前正式发送链路上。到这里，当前产品口径与代码行为已经对齐为：Shizuku 只保留系统授权与 AppOps 诊断辅助，不再属于正式读剪贴板主通道
- 本轮还继续把无障碍兼容态的运行页文案再压实了一层，而不只是改 Shizuku：`ClipboardModeSupport.kt` 与 `MainActivity.kt` 里原先残留的“无障碍辅助能力”表述，现已统一收口成“无障碍辅助链路”，包括已就绪摘要、当前辅助状态、监听策略、阻塞提示和建议文案。这样后续用户在运行页里更容易区分“正式模式”和“仅兼容旧配置保留的辅助链路”，不会再把无障碍兼容态误读成当前正式主模式
- 这轮 Android 本地验证也补充了一个后续必须记住的流程约束：中途曾因为把 `./gradlew.bat testDebugUnitTest --no-daemon` 和 `./gradlew.bat assembleDebug --no-daemon` 并行启动，触发 Windows 下同一 `app/build/intermediates` 被双任务同时占用，出现 `values-hi.xml` 文件锁与 `mergeDebugResources` 路径不存在这类假故障。清理 `android-sync-client/app/build`、`android-sync-client/build` 并停止 daemon 后，改回串行执行 `testDebugUnitTest -> assembleDebug` 已重新通过。因此后续 Android 本地回归要继续坚持串行执行，不要再把同一工作目录下的 Gradle 任务并行跑
...
- 本轮继续把 Android 旧模式收口从“只改文案”推进到“文案与行为一致”：`android-sync-client` 的 `SyncService` 已移除 `shizuku` 模式下那两处仍会实际调用 `ShizukuClipboardReader` 的正式读剪贴板分支，当前正式同步与手动发送读取都统一回到系统 `ClipboardManager` / 既有无障碍快照补偿主链路；`Shizuku` 现在真正只保留为系统授权与剪贴板 AppOps 的诊断辅助，不再一边在运行页写“只做诊断”，一边又在服务里偷偷走独立读剪贴板分支
- 本轮也把 Android 运行页里残留的“无障碍辅助能力”进一步统一成“无障碍辅助链路”口径，覆盖 `ClipboardModeSupport.kt`、`MainActivity.kt` 的实现摘要、当前辅助状态、监听策略与阻塞说明，避免用户继续把历史兼容态误读成正式主模式能力
- 这轮本地验证需要特别记住一个环境事实，避免后续再误判：中途我一度把 `android-sync-client` 的 `testDebugUnitTest` 和 `assembleDebug` 并行跑在同一工作目录，Windows 下 Gradle / Kotlin 增量编译会直接互相踩 `build/intermediates` 与资源合并目录，先后出现 `AccessDeniedException`、`mergeDebugResources` 文件占用、路径不存在和一串看似离谱的 Kotlin 断引用报错；在执行 `gradlew --stop`、删除 `android-sync-client/app/build` 与 `android-sync-client/build` 后，改回串行执行 `./gradlew.bat testDebugUnitTest --no-daemon` 再执行 `./gradlew.bat assembleDebug --no-daemon`，两者均已恢复通过，说明这次异常属于并行验证造成的 Windows 文件锁假故障，而不是这轮源码改动引入的新回归
- 本轮继续把 `floating` 从“可手动打开的小工具”推进到更贴近正式路线的行为：当 Android 当前正式模式切到 `clipboard_mode=floating` 且悬浮窗权限已就绪时，`SyncService.publishLocalClipboardIfNeeded(...)` 现在检测到新的本地纯文本后，不再沿用前台模式那套立即直推逻辑，而是更新诊断状态并主动弹出 `FloatingClipboardOverlayService`；真正发送仍要用户在悬浮发送助手里明确点一次“发送文本”。这条行为与当前产品定义已对齐：`floating` 负责“复制后的快速发送入口”，强调显式用户动作优先，避免误发普通打字
- 本轮最新真机联调已经把 Android 当前主链路收出一条明确结论：在新版 debug 包覆盖安装后，`clipboard_mode=floating` + 当前真实配置下，手机侧 `bootstrap` 心跳会正常刷新 `lastSeenAt`，并且后续已进一步恢复到 `online=true`，说明此前“前台服务还在但服务端一直显示 offline”的问题已经被压缩到旧包/旧状态；继续通过 `DebugPublishActivity --es extra_action manual-send --es extra_text ...` 触发显式发送后，服务端 `api/sync/status?room=default` 与 `bootstrap` 已真实出现 `codex-manual-via-activity-20260613-1` 这条 `recentMessages`，证明当前 Android -> 服务端 的显式文本发送主链路已经重新打通
- 同一轮也确认了一条需要后续继续跟进、但已不再阻塞主链路判断的边界：`DEBUG_SEND_CLIPBOARD` 广播入口在当前真机现场下没有成功把文本送入待发送队列，而 `DebugPublishActivity` 这条更直接的调试入口可以稳定命中 `recentMessages`。后续若继续做自动化联调，应优先复用 `DebugPublishActivity` 或正式 UI/分享入口验证发送闭环，不要再把广播入口当成唯一证据
- 本轮 Windows 本机构建环境也新增一条必须记住的经验：当前工作区与直接复制到 `C:\Temp` 的 Android 工程目录，在 Windows 上都可能命中 `mergeDebugResources` 对 `app/build/intermediates/.../values*.xml` 的文件锁/路径不存在假故障；但同一份临时代码在隔离目录中重新完整构建时，`assembleDebug` 已成功通过，并且由这份 APK 安装到真机后完成了上面的在线与发送闭环。因此后续如果本机继续撞上这类资源合并锁，应优先按“停止 daemon / 删除 build / 必要时改到隔离目录构建”的方式处理，不要把它误判成业务代码回归
- 本轮继续把 Shizuku 自动读剪贴板这条探索线做了最后一轮真机判定：`android-sync-client` 已重新 `assembleDebug` 并覆盖安装到真机 `4e9e24e7`，随后通过 `DebugClipboardInjectReceiver` 成功写入测试文本 `codex-shizuku-smoke-20260615-1`，再由 `SyncService` 在 `clipboard_mode=shizuku` 下触发自动轮询读取。日志显示 `Shizuku` 授权、服务在线、系统剪贴板服务 binder 正常，但 `getUserPrimaryClip / getPrimaryClip / getStashPrimaryClip` 以及 binder transaction、remote process 兜底最终都返回空文本或 `Parcel data not fully consumed`。也就是说，这条路线在当前 ROM / 设备现场仍然拿不到可发送文本，不应再把它当成已可用的后台读剪贴板主通道
- 这轮真机判定后，计划口径继续维持当前收口不变：`Shizuku` 只保留系统授权与 AppOps 诊断辅助，不再推进为正式读剪贴板主通道；后续要继续补的是 `foreground / floating / 不替换原键盘的新方案` 和真实可用的发送兜底入口，而不是继续在 Shizuku 读法上做产品化打磨
- 本轮继续针对 Shizuku 辅助模式的 runtime 噪音做了收口，不再让失败探针在轮询里反复刷屏：`ShizukuClipboardReader` 已加入短时失败退避，`SyncService` 仍保留 listener / 显式触发时的重试，但 `poll` 路径在连续失败后会先安静退避一小段时间，避免真机日志和运行页提示每 1.5 秒重复刷同一条 binder / remote-process 失败摘要。对应本地 `android-sync-client` 仍已通过 `./gradlew.bat testDebugUnitTest --no-daemon` 与 `./gradlew.bat assembleDebug --no-daemon`
- 本轮又把 Shizuku 辅助模式从“只会失败后退避”继续推进了一步：`ShizukuClipboardReader` 现在把深探针收口成仅显式手动探测时才允许，`poll / listener` 只做轻量直接读取；而 `SyncService.publishClipboardThroughShizukuAssist(...)` 在 Shizuku 读取未命中后，会继续尝试无障碍快照补传，并在有悬浮窗权限时直接弹出悬浮发送助手作为稳定兜底。这样 Shizuku 这条辅助路径至少会沿着“读取 -> 快照兜底 -> 悬浮回退”继续往前走，而不是卡在一条失败信息里
- 本轮继续把这条兜底链路和配置对齐：`SyncService.publishClipboardThroughShizukuAssist(...)` 现在会严格按 `shizukuFallbackFloatingEnabled` 决定是否自动打开悬浮发送助手，开启时且悬浮窗权限可用才会弹窗；关闭时则只保留轻量提示，不再出现“设置里关了但运行时仍自动弹出”的打架状态。这样后续对 Shizuku 辅助模式的预期会更一致，也更方便继续做真机联调
- 本轮又把 Shizuku 辅助模式的失败回退再收紧了一层，专门解决“同一失败态在 `poll` / `listener` 两条路里被反复弹出”的问题：`SyncService.publishClipboardThroughShizukuAssist(...)` 现在的冷却签名不再把 `source` 算进去，而是按失败原因整体去重；`FloatingClipboardOverlayService` 也补了 `showing` 状态和重复显示早退，避免 overlay 自己在短时间内被再次 `startService` 时反复重开。后续继续联调时，Shizuku 失败应只保留一次悬浮兜底和轻量提示，而不再因为轮询和监听同时命中而连环刷窗
- 本轮把上面那条“只保留一次悬浮兜底”的逻辑又向前补实了一步：`FloatingClipboardOverlayService` 和 `FloatingConfirmService` 现在都改成了可在 Android 14 真机后台安全拉起的前台服务，并在 Manifest 里补了 `foregroundServiceType="dataSync"`；`DebugClipboardInjectReceiver` 的 Shizuku smoke 入口也改回正确的 `SyncService.ACTION_DEBUG_SHIZUKU_SMOKE`，不再把 action 名字发岔。真机回归结果是：`DEBUG_SHIZUKU_SMOKE` 触发后只出现一次 Shizuku 失败兜底悬浮助手，随后按 60 秒级退避进入轻提示，没有继续刷窗
- 这轮又补了一层接收页防抖：`PayloadEntry` 新增 `lastNotifiedAt`，`SyncService.onPayloadNotice(...)` 会先判断同一条 payload 是否已经展示过，展示过的 notice 回放将直接跳过自动弹窗，只保留列表里的手动查看入口；这样服务端 recent / reconnect 再回放同一批历史接收卡时，不会继续把接收页一遍遍顶到前台。当前真机上的 3 条 debug `codex-final-receive.png` payload 也已被标记为已展示，避免旧调试残留继续打扰
- 这轮继续把这条去重收口做成了可复现的真机证据：用服务端正式 `/api/sync/payload-notice` 接口推送同一个 debug `payloadId=debug-lastnotif-test-001`，Android 第一次收到时日志命中 `shouldPresent=true`，随后同一 `payloadId` 再次回放立即变成 `shouldPresent=false`；强制重启 App 后，重连日志仍然是 `shouldPresent=false`。这说明 `lastNotifiedAt` 的持久化去重已经生效，不会因为进程重启或 WebSocket 重连而再次把旧接收卡顶到前台
