...
- Android 真机后台复制与权限受限场景体验已经打通 Chrome 后台复制验证，Shizuku 安装/运行/授权状态也已接入真机验证；Shizuku 已授权时现在可作为系统授权与剪贴板 AppOps 诊断辅助模式启动；Android 16 真机巡检显示通知、Shizuku API、剪贴板 AppOps 和电池白名单可用，并已为无障碍启用状态增加 AccessibilityManager 兜底判断与来源详情展示，权限页和运行页都会显示无障碍状态来源；Android 通知渠道名和渠道说明已改为中文展示，权限页已补充运行状态通知与接收提醒通知的直达入口，后续继续覆盖更多真实 App 的长按复制、输入框复制和厂商剪贴板浮窗场景，并继续评估 Shizuku 独立剪贴板主通道
