...
**⚠️ Android 13+ 后台剪贴板限制：**
- 从 Android 13 (API 33) 开始，系统禁止后台应用访问剪贴板（隐私保护特性）
- **影响范围**：floating（悬浮窗）和 ime_background（原键盘后台）模式的后台复制
- **实际表现**：App 在后台时复制文本，需要切回 App 前台才能触发同步
- **前台使用**：App 在前台或最近使用时，所有模式均正常工作 ✅
- **解决方案**：
  1. 推荐：保持 App 在最近任务（切回即可触发同步）
  2. 进阶：使用 [Shizuku](https://shizuku.rikka.app/) 提权绕过限制（需额外配置）
- **说明**：这是 Android 系统级限制，同类产品也受此约束
