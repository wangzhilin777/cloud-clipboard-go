...
本项目在原有网页剪贴板和文件中转能力基础上，新增并收口了以下能力：
...
| 文件 / 图片通知发送 | 支持 | 支持 | 以接收为主 | 网页端和桌面端可上传文件后发送 `payloadNotice` |
| 文件 / 图片确认接收 | 不做自动接收 | 不做自动接收 | 支持 | 非文本自动接收由 Android 端处理，且必须用户确认后才下载 |
...
**✅ Android 13+ 后台剪贴板限制已解决：**
- 从 Android 13 (API 33) 开始，系统禁止后台应用访问剪贴板（隐私保护特性）
- **推荐方案**：使用 **Shizuku 后台模式**（真正的后台自动同步）
  1. 安装 [Shizuku](https://shizuku.rikka.app/) 并启动服务（支持 root 或无线调试）
  2. 在云剪同步 App 中选择"Shizuku 后台"模式
  3. 授权云剪同步使用 Shizuku 权限
  4. App 在后台时复制文本，自动同步到其他设备（< 1 秒延迟）✅
- **备用方案**：使用 **ime_background（原键盘后台）模式**
  1. 在系统设置中启用"云剪同步"输入法（不需要切换为默认输入法）
  2. 在 App 中选择"原键盘后台发送"模式
  3. 仅当"云剪同步"输入法激活时，可在后台读取剪贴板
  4. 注意：切换到其他输入法时，后台读取会失效
- **效果对比**：
  - Shizuku 模式：真正的后台自动同步，不受输入法影响 ✅✅✅
  - IME 模式：仅在自身输入法激活时可后台同步，实用性受限
  - 前台使用：所有模式在 App 前台时均正常工作 ✅
- **详细说明**：见 [Android 13+ 后台剪贴板限制解决方案](./docs/05-android-background-solution.md)
...
- 入口脚本仍兼容旧变量名 `ROOM_AUTH`，但 Compose 示例和相关文档统一使用 `ROOM_AUTH_JSON`。
...
- 控制面板概览页可直接查看下一步建议、连接诊断、缓存目录、最近缓存清理、平台能力、最近远端文本同步、最近远端文件通知与状态最近刷新
